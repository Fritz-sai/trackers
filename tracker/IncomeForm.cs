    ...
    public partial class IncomeForm : Form
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DatabaseConnection.OpenConnection();
                string query = "INSERT INTO income (amount, category, date, description) VALUES (@amount, @category, @date, @description)";
                using (MySqlCommand cmd = new MySqlCommand(query, DatabaseConnection.GetConnection()))
                {
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DatabaseConnection.OpenConnection();
                string query = "UPDATE income SET amount = @amount, category = @category, description = @description WHERE id = @id";
                using (MySqlCommand cmd = new MySqlCommand(query, DatabaseConnection.GetConnection()))
                {
                    cmd.Parameters.AddWithValue("@amount", decimal.Parse(textBox3.Text));
                    cmd.Parameters.AddWithValue("@category", comboBox1.Text);
